'use client';
// app/menu/page.tsx — Public QR-scannable customer menu (no auth required)
    ...
    <div className="min-h-screen bg-zinc-950">
      ...
      <div className="max-w-xl mx-auto px-4 py-4">
                ...
                <div className="p-3">
                  ...
                  {item.tags && item.tags.length > 0 && (
                    <div className="flex gap-1 mt-2 flex-wrap">
                      {item.tags.map(tag => (
                        <span key={tag} className="bg-zinc-800 text-zinc-400 text-[9px] px-1.5 py-0.5 rounded-full">{tag}</span>
                      ))}
                    </div>
                  )}
                </div>
        ...
        <p className="text-center text-zinc-700 text-xs mt-8 pb-4">
          Powered by foodashh POS · To order, please speak to your waiter.
        </p>
      </div>
    </div>
  );
